...
async def gitlab_webhook(request: Request, x_gitlab_token: str | None = Header(default=None)) -> Response:
    ...
    if settings.elasticsearch_enabled and settings.elasticsearch_url:
        elastic = ElasticService(settings.elasticsearch_url, settings.elasticsearch_index_prefix, settings.elasticsearch_api_key or None)
        try:
    ...
    if not inserted and existing_key:
        return Response(status_code=204)

    if settings.queue_enabled:
        await QueueRepository(settings.database_url).enqueue("jira.manual_job", payload)
